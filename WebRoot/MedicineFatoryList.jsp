   ...
   function SearchMedicineFatory()
	...
	function deleteChecked(checkboxName){
		...
		if(param.length >0){
			...
			if(bool){
				//这里提交到删除的地方
				window.location.href="${pageContext.request.contextPath}/MedicineFatory_DelMore?ID="+param;
			}
		}
...
<table id="list_table" >
      ...
      <tr>
        ...
        <td>
          ...
          <a href="javascript:deleteMedicinefatory('<s:property value="id" />')">删除</a> 
          </s:if>
          <s:elseif test='type=="price"'>
          <a href="javascript:getMedicinefatory('<s:property value="id" />')">修改售价</a>
          </s:elseif>
       </td>   
      </tr>
      </s:iterator>
...
<div id="page_bar">
		...
		<div id="page">
			...
			<div id="tool">
					...
					<s:else>
						<a href="${pageContext.request.contextPath}/MedicineFatoryList?page=1">第一页</a>
						<a href="${pageContext.request.contextPath}/MedicineFatoryList?page=<s:property value="%{pageBean.currentPage-1}"/>">上一页</a>
					</s:else>
...
<td><a href="javascript:SearchMedicineFatory()"/>搜索</td></tr>
</table>
</s:if>
</body>
</html>
